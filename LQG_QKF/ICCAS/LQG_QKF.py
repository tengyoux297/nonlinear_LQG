...
class LQG:
    ...
    def update_lqr_aug(self, goal_state):
...
def one_trial(H=1000, noise_scale=1e-1, m_scale=1e2, Q_scale=1.0, R_scale=1.0, rand_seed=None):
    ...
    return [err_list_qkf, var_list_qkf, time_list_qkf], [err_list_ekf, var_list_ekf, cost_list_ekf, time_list_ekf], [err_list_aug_num, var_list_aug_num, cost_list_aug_num, time_list_aug_num], [err_list_ukf, var_list_ukf, cost_list_ukf, time_list_ukf]
...
def test(H=1000, trials=20, plot=True, noise_scale=1e-1, m_scale=1e2, Q_scale=1.0, R_scale=1.0, rand_seed=None):
    ...
    if (os.path.exists(ekf_file) and os.path.exists(qkf_file) and   
        os.path.exists(ukf_file)):
        print(f"Found existing pkl files for m_scale={int(m_scale)}. Skipping simulation.")
        print(f"Loading existing results from: {ekf_file}, {qkf_file}, {ukf_file}")
        ...
        for i in tqdm(range(trials)):
            ...
            lqe_qkf_results, ekf_results, qkf_num_results, ukf_results = one_trial(
    ...
    err_list_ekf_std = np.std(np.array(err_list_ekf_all), axis=0)
    
    var_list_ekf_avg = np.mean(np.array(var_list_ekf_all), axis=0)
    var_list_ekf_std = np.std(np.array(var_list_ekf_all), axis=0)
    
    cost_list_ekf_avg = np.mean(np.array(cost_list_ekf_all), axis=0)
    cost_list_ekf_std = np.std(np.array(cost_list_ekf_all), axis=0)
    
    time_list_ekf_avg = np.mean(np.array(time_list_ekf_all), axis=0)
    time_list_ekf_std = np.std(np.array(time_list_ekf_all), axis=0)

    err_list_qkf_num_avg = np.mean(np.array(err_list_qkf_num_all), axis=0)
    err_list_qkf_num_std = np.std(np.array(err_list_qkf_num_all), axis=0)
    
    var_list_qkf_num_avg = np.mean(np.array(var_list_qkf_num_all), axis=0)
    var_list_qkf_num_std = np.std(np.array(var_list_qkf_num_all), axis=0)
    
    cost_list_qkf_num_avg = np.mean(np.array(cost_list_qkf_num_all), axis=0)
    cost_list_qkf_num_std = np.std(np.array(cost_list_qkf_num_all), axis=0)
    
    time_list_qkf_num_avg = np.mean(np.array(time_list_qkf_num_all), axis=0)
    time_list_qkf_num_std = np.std(np.array(time_list_qkf_num_all), axis=0)

    err_list_ukf_avg = np.mean(np.array(err_list_ukf_all), axis=0)
    err_list_ukf_std = np.std(np.array(err_list_ukf_all), axis=0)
    
    var_list_ukf_avg = np.mean(np.array(var_list_ukf_all), axis=0)
    var_list_ukf_std = np.std(np.array(var_list_ukf_all), axis=0)
    
    cost_list_ukf_avg = np.mean(np.array(cost_list_ukf_all), axis=0)
    cost_list_ukf_std = np.std(np.array(cost_list_ukf_all), axis=0)
    
    time_list_ukf_avg = np.mean(np.array(time_list_ukf_all), axis=0)
    time_list_ukf_std = np.std(np.array(time_list_ukf_all), axis=0)
    ...
    if plot:  
        ...
        ax[0].plot(err_list_qkf_num_avg, label='QKF', color=PUBLICATION_COLORS['qkf_numeric'], linewidth=2.5)
        ...
        ax[1].plot(var_list_qkf_num_avg, label='QKF', color=PUBLICATION_COLORS['qkf_numeric'], linewidth=2.5)
        ukf_line = ax[1].plot(var_list_ukf_avg, label='UKF', color=PUBLICATION_COLORS['ukf'], linewidth=2.5)[0]
        handles, labels = ax[1].get_legend_handles_labels()
        # Reorder legend to original: EKF, UKF, QKF
        legend_order = [labels.index('EKF'), labels.index('UKF'), labels.index('QKF')]
        ...
        avg_times = [np.mean(time_list_ekf_avg), np.mean(time_list_ukf_avg), np.mean(time_list_qkf_num_avg)]
        controllers = ['LQG+EKF', 'LQG+UKF', 'iLQG+QKF']
        colors = [PUBLICATION_COLORS['ekf'], PUBLICATION_COLORS['ukf'], PUBLICATION_COLORS['qkf_numeric']]
        ...
        ax.set_title(f'Cost Performance Comparison, Nonlinearity Scale = {int(m_scale)}',
                    fontsize=20, fontweight='bold', pad=20)
        ax.set_xlabel('Time Step', fontsize=20, fontweight='bold')
        ax.set_ylabel('Cost-to-go (log scale)', fontsize=20, fontweight='bold')

        # Compute quantiles for each filter method
        ekf_q10 = np.percentile(cost_list_ekf_all, 10, axis=0)
        ekf_q90 = np.percentile(cost_list_ekf_all, 90, axis=0)

        ukf_q10 = np.percentile(cost_list_ukf_all, 10, axis=0)
        ukf_q90 = np.percentile(cost_list_ukf_all, 90, axis=0)

        qkf_q10 = np.percentile(cost_list_qkf_num_all, 10, axis=0)
        qkf_q90 = np.percentile(cost_list_qkf_num_all, 90, axis=0)

        x = np.arange(len(cost_list_ekf_avg))

        # --- Quantile ribbons (90–10 percentile) ---
        ax.fill_between(x, ekf_q10, ekf_q90, color=PUBLICATION_COLORS['ekf'], alpha=0.15, linewidth=0)
        ax.fill_between(x, ukf_q10, ukf_q90, color=PUBLICATION_COLORS['ukf'], alpha=0.15, linewidth=0)
        ax.fill_between(x, qkf_q10, qkf_q90, color=PUBLICATION_COLORS['qkf_numeric'], alpha=0.15, linewidth=0)

        # --- Mean lines (on top) ---
        ax.plot(x, cost_list_ekf_avg, label='LQG+EKF', color=PUBLICATION_COLORS['ekf'], linewidth=2.5, linestyle='-')
        ax.plot(x, cost_list_ukf_avg, label='LQG+UKF', color=PUBLICATION_COLORS['ukf'], linewidth=2.5, linestyle='--')
        ax.plot(x, cost_list_qkf_num_avg, label='iLQG+QKF', color=PUBLICATION_COLORS['qkf_numeric'], linewidth=2.5, linestyle='-.')

        # Legend, grid, and style adjustments
        ax.legend(
            frameon=True,
            fancybox=True,
            shadow=True,
        ...
        plt.tight_layout()
        plt.savefig(perf_dir + f'cost_perf-mscale={int(m_scale)}.png',
        ...
        ax.plot(time_list_qkf_num_avg, label='iLQG+QKF', color=PUBLICATION_COLORS['qkf_numeric'], linewidth=2.5, linestyle='-.')
        ...
        print(f"iLQG+QKF - Avg time per step: {np.mean(time_list_qkf_num_avg):.4f} ± {np.std(time_list_qkf_num_avg):.4f} seconds")
        print(f"Total simulation time: LQG+EKF {np.sum(time_list_ekf_avg):.2f}s, LQG+UKF {np.sum(time_list_ukf_avg):.2f}s, iLQG+QKF {np.sum(time_list_qkf_num_avg):.2f}s")

    return cost_list_ekf_avg, cost_list_ukf_avg, cost_list_qkf_num_avg
...
def nonlinearity_test(H=1000, trials=20):
    ...
    for i, m_scale in enumerate(m_scales):
        ...
        cost_list_ekf_avg, cost_list_ukf_avg, cost_list_qkf_num_avg = test(H=H, trials=trials, plot=False, m_scale=m_scale, rand_seed=rand_seed)
...
def plot_nonlinearity_analysis(results):
    ...
    ax.loglog(results['m_scales'], results['qkf_numeric_costs'], 
               label='iLQG+QKF', color=PUBLICATION_COLORS['qkf_numeric'], 
    ...
    print(f"QKF average costs: {[f'{cost:.2e}' for cost in results['qkf_numeric_costs']]}")
